...
class KCS215005_030206_04(RuleUnit):
...
flowchart TD
    ...
    @rule_method
    def allowable_deviation_between_adjacent_forms(sISurGra) -> str:
        """인접한 거푸집의 어긋남 허용오차

        Args:
            sISurGra (string): 표면 평탄하기 등급

        Returns:
            fORelTol (float): 상대오차
        """

        if sISurGra == "A급" or sISurGra == "A":
            fORelTol = 3
            return fORelTol
        if sISurGra == "B급" or sISurGra == "B":
            fORelTol = 6
            return fORelTol
        if sISurGra == "C급" or sISurGra == "C":
            fORelTol = 13
            return fORelTol
