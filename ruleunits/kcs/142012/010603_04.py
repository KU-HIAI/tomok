...
class KCS142012_010603_04(RuleUnit):
    ...
    def general_pressure(fIW, fIH) -> float:
        """
        Args:
            fIW (float): 굳지 않은 콘크리트의 단위 중량
            fIH (float): 콘크리트의 타설 높이
        Returns:
            fOGenPre (float): 일반 콘크리트 측압
        """
        fOGenPre = fIW * fIH
        return fOGenPre

    def column_pressure(self, fIW, fIH, sICemAdm, fIR, fIT) -> float:
        """
        Args:
            fIW (float): 굳지 않은 콘크리트의 단위 중량
            fIH (float): 콘크리트의 타설 높이
            sICemAdm (string): 시멘트 종류 및 첨가물
            fIR (float): 콘크리트 타설 속도
            fIT (float): 콘크리트의 온도
        Returns:
            fOPreCol (float): 기둥의 측압
        """
        if fIW <= 22.5:
        ...
        if fIR <= 2.1 and fIH < 4.2:
            fOPreCol = min(max(C_w * C_c * (7.2 + (790 * fIR) / (fIT + 18)), 30 * C_w), fIW * fIH)
        else:
            fOPreCol = min(max(C_w * C_c * (7.2 + (1160 + 240 * fIR) / (fIT + 18)), 30 * C_w), fIW * fIH)
        return fOPreCol

    def wall_pressure(self,fIW, fIH, sICemAdm, fIR, fIT) -> float:
        """
        Args:
            fIW (float): 굳지 않은 콘크리트의 단위 중량
            fIH (float): 콘크리트의 타설 높이
        ...
        if sICemAdm == "지연제를 사용하지 않은 KS L 5201의 1, 2, 3종 시멘트":
            ...
            C_c = 1.4
        if fIR <= 2.1 and fIH < 4.2:
            fOPreWal = min(max(C_w * C_c * (7.2 + (790 * fIR) / (fIT + 18)), 30 * C_w), fIW * fIH)
        else:
            fOPreWal = min(max(C_w * C_c * (7.2 + (1160 + 240 * fIR) / (fIT + 18)), 30 * C_w), fIW * fIH)
        return fOPreWal

    def lateral_pressure(self, fIConSlu, fIPouDep, bIVibCom, bICol, bIWal, fIW, fIH, sICemAdm, fIR, fIT) -> float:
        """
        Args:
            fIConSlu (float): 콘크리트 슬럼프
            fIPouDep (float): 타설깊이
            bIVibCom (boolean): 일반적인 내부 진동다짐
            bICol (boolean): 기둥
            bIWal (boolean): 벽체
            fIW (float): 굳지 않은 콘크리트의 단위 중량
            fIH (float): 콘크리트의 타설 높이
        ...
        if fIConSlu <= 175 and fIPouDep <= 1.2 and bIVibCom and bICol:
            result = self.column_pressure(fIW, fIH, sICemAdm, fIR, fIT)
        elif fIConSlu <= 175 and fIPouDep <= 1.2 and bIVibCom and bIWal:
            result = self.wall_pressure(fIW, fIH, sICemAdm, fIR, fIT)
        else:
            result = self.general_pressure(fIW, fIH)
        return result