...
class KCS244005_030103(RuleUnit):
    ...
    @rule_method
    def PTFE_fiber(sIBriTyp,fIB,fIDesMov,sIRevIte)->str:
        """
        Args:
            sIBriTyp (string): 교량 종류
            fIB (float): 받침 중심간격
            fIDesMov (float): 설계이동량
            sIRevIte (string): 검사항목
        Returns:
            sOBeaIns (string): 받침 설치 검사기준
        """
        if sIRevIte == "받침 중심간격(교축직각방향)":
            if sIBriTyp == "콘크리트교":
                ...
                return "교량 종류는 콘크리트교와 강교 중에 선택해주세요"
            ...
            return "검사항목은 받침 중심간격(교축직각방향), 가동받침의 이동가능량, 가동받침의 교축방향의 이동편차 동일 받침선 상의 상대오차, 설치 높이,교량 전체 받침의 상대높이 오차, 단일 box를 지지하는 인접 받침의 상대높이 오차, 포트받침의 수평도(교축 및 직각방향), 기타받침의 수평도(교축 및 직각방향), 앵커볼트의 연직도 중에서 선택해주세요."
        return sOBeaIns