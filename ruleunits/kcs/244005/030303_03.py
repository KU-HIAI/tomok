...
class KCS244005_030303_03(RuleUnit):
    priority = 1
    author = '서울대학교, 최정운'
    ref_code = 'KCS 24 40 05 3.3.3 (3)'
    ref_date = '2018-08-30'
    doc_date = '2023-11-07'
    title = '디스크받침의 하중 시험'
    ...
    ③ 받침은 시험동안 그리고 시험 후에 분해하여 육안으로 검사하여야 한다. 돌출되거나 변형된 폴리에테르 우레탄 또는 PTFE, 손상된 구속링 또는 균열이 발생한 강재 등과 같이 육안으로 관찰되는 결함은 불합격의 원인이 된다. 시험하는 동안에 폴리에테르우레탄 디스크와 받침판 사이에서 그리고 상부 미끄럼 강판과 상부 받침판 사이에서 연속적이고 균일한 접촉이 유지되어야 한다. 들뜬 것이 발견되면, 이것은 해당 로트의 불합격 요인이 된다.
    """
    ...
    @rule_method
    def load_test(fIDesLoa,fIDesRot):
        """
        Args:
        ...
        return "디스크받침의 하중 시험: " + str(fOLoaTes) + " kN" + ", 디스크받침의 회전 시험: " + str(fORotTes) + " rad"