...
class KCS243000_030201_05(RuleUnit):
    ...
    @rule_method
    def welding_test(fISteThi,bIRolWel,bIWeaHot,bIShiArc,bIGasMet,bISubArc,bIFCA,sISteTyp,fIHeaPas,bIMeeReq,bIHisUse)-> str:
        """
        Args:
            fISteThi (float): 강판두께
            bIRolWel (boolean): 용접구조용 압연강재
            bIWeaHot (boolean): 내후성 열간압연강재
            bIShiArc (boolean): 피복아크용접(수용접의 경우만)
            bIGasMet (boolean): 가스메탈 아크용접
            bISubArc (boolean): 서브머지드 아크용접
            bIFCA (boolean): 플럭스코어드 아크용접
            sISteTyp (string): 강종
            fIHeaPas (float): 패스의 입열량
            bIMeeReq (boolean): 기준의 요건을 충족
            bIHisUse (boolean): 사용실적이 없는 공급원이 공급한 재료
        Returns:
            sOWelTes (string): 용접시공시험
        """
