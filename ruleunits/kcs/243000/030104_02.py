...
class KCS243000_030104_02(RuleUnit):
    ...
    @rule_method
    def gas_cutting_processing(sIMemTyp,bISurRou,bINotDep,bISla,bIConEdg)-> str:
        """
        Args:
            sIMemTyp (string): 부재의 종류
            bISurRou (boolean): 표면거칠기
            bINotDep (boolean): 노치 깊이
            bISla (boolean): 슬래그
            bIConEdg (boolean): 절단된 모서리의 상태
        Returns:
            sOCutTol (string): 주요 부재의 절단 도구
            sOTolPla (string): 가스절단 및 가스 가공한 강판의 허용오차
            sOQuaSur (string): 절단면의 품질
        """
        sOCutTol = "자동가스절단기"
        ...
        if bISurRou:
            if sIMemTyp == "주요 부재":
                ...
                return "부재의 종류는 주요 부재, 2차 부재 중에서 선택해주세요"
        elif bINotDep:
            if sIMemTyp == "주요 부재":
                sOQuaSur = "노치가 없어야 한다."
            elif sIMemTyp == "2차 부재":
            ...
            return "표면거칠기, 노치 깊이, 슬래그, 절단된 모서리의 상태 중 하나를 선택해주세요"
        return "주요 부재의 절단 도구: " + sOCutTol + " 가스절단 및 가스 가공한 강판의 허용오차: " + sOTolPla + " 절단면의 품질: " + sOQuaSur

    def cutting_rolled_steel(self, fIThiExt)-> str:
        """
        Args:
            fIThiExt (float): 극후판의 두께
        Returns:
            sOCutRol (string): 압연강재의 절삭처리
        """
        if fIThiExt > 50:
    ...
    def quality_cutting_surface(self,sIMemTyp,fISurRou,fINotDep)->str:
        """
        Args:
            sIMemTyp (string): 부재의 종류
