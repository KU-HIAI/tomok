...
class KDS143110_0401050101 (RuleUnit):
    ...
    ref_code = 'KDS 14 31 10 4.1.5.1.1' # 건설기준문서
    ref_date = '2017-12-20'  # 디지털 건설문서 작성일
    doc_date = '2023-12-12'  # 건설기준문서->디지털 건설기준 변환 기준일
    title = '인장파단에 대한 공칭인장강도'    # 건설기준명
    ...
    @rule_method
    def Nominal_tensile_strength_to_tensile_rupture(fOPn,fIt,fIbeff,fIFu) -> float:
        """인장파단에 대한 공칭인장강도

        Args:
            fOPn (float): 인장파단에 대한 공칭인장강도
            fIt (float): 판재의 두께
            fIbeff (float): 유효연단거리
            fIFu (float): 인장강도

        Returns:
            float: 강구조부재설계기준(하중저항계수설계법)  4.1.5.1.1 유효순단면적에 대한 인장파단의 값
        """
