...
class KDS143125_04010303_01 (RuleUnit):
    ...
    ref_code = 'KDS 14 31 25 4.1.3.3 (1)' # 건설기준문서
    ref_date = '2017-12-20'  # 고시일
    doc_date = '2023-09-14'  # 건설기준문서->디지털 건설기준 변환 기준일
    title = '볼트의 인장 및 전단강도'    # 건설기준명
    ...
    @rule_method
    def Tensile_and_shear_strength_of_bolt(fOphiRn,fIphi,fIFn,fIAb) -> bool:
        """볼트의 인장 및 전단강도

        Args:
            fOphiRn (float): 설계인장강도 또는 설계전단강도
            fIphi (float): 저항계수
            fIFn (float): 공칭인장강도
            fIAb (float): 볼트, 또는 나사 강봉의 나사가 없는 부분의 공칭단면적

        Returns:
            bool: 강구조 연결 설계기준(하중저항계수설계법)  4.1.2.4 설계원칙 (1)의 통과 여부
        """
