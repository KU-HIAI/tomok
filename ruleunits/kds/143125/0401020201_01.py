...
class KDS143125_0401020201_01 (RuleUnit):
    ...
    ref_code = 'KDS 14 31 25 4.1.2.2.1 (1)' # 건설기준문서
    ref_date = '2017-12-20'  # 고시일
    doc_date = '2023-09-08'  # 건설기준문서->디지털 건설기준 변환 기준일
    title = '필릿용접의 유효면적'    # 건설기준명
    ...
    @rule_method
    def Effective_area_of_fillet_weld(fOefarfw,fIefflen,fIefthth) -> bool:
        """필릿용접의 유효면적

        Args:
            fOefarfw (float): 필릿용접의 유효면적
            fIefflen (float): 유효길이
            fIefthth (float): 유효목두께

        Returns:
            bool: 강구조 연결 설계기준(하중저항계수설계법)  4.1.2.2.1 필릿용접의 유효면적 (1)의 값
        """
