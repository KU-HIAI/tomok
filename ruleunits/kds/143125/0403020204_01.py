...
class KDS143125_0403020204_01 (RuleUnit):
    ...
    ref_code = 'KDS 14 31 25 4.3.2.2.4 (1)' # 건설기준문서
    ...
    flowchart TD
  	...
  	VarIn4[/입력변수: 오버랩 접합계수/] ;
    VarIn5[/입력변수: 겹치는 지강관의 높이/]
    VarIn6[/입력변수: 주강관에 용접된 지강관 면의 유효폭/] ;
    VarIn7[/입력변수: 겹침 브레이스에 용접된 지강관 면의 유효폭/] ;
    ...
    @rule_method
    def Design_strength_of_steel_pipe(fOPhiPn,fIPn,fIFybi,fItbi,fIOv,fIHbi,fIbeoi,fIbeov) -> bool:
        ...
        Returns:
            bool: 강구조 연결 설계기준(하중저항계수설계법) 4.3.2.2.4 겹침 K형 접합에서 압축력을 받는 지강관 (1)의 값
        """

        if 25 <= fIOv <= 50:
          fIPn = fIFybi * fItbi * ((fIOv / 50) * (2 * fIHbi - 4 * fItbi) + fIbeoi + fIbeov)
          fOPhiPn = 0.95 * fIPn
          return fOPhiPn
        else:
          return "Fail"
