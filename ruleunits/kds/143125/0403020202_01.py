...
class KDS143125_0403020202_01 (RuleUnit):
    ...
    ref_code = 'KDS 14 31 25 4.3.2.2.2 (1) ' # 건설기준문서
...
U --"Minimum"-->W(["<img src='https://latex.codecogs.com/svg.image?\phi&space;P_{n}'>--------------------"])
    ...
    @rule_method
    def Limit_state_of_plasticization_of_cast_steel_pipe_wall(fOPnsintheta,fIFy,fIt,fIeta,fIbeta,fIQf) -> bool:
        """주강관벽 소성화의 한계상태
        Args:
        ...
        Returns:
            bool: 강구조 연결 설계기준(하중저항계수설계법) 4.3.2.2.2 T, Y, X형 접합의 압축력을 받는 지강관 (1)의 통과여부
        """

        fOPnsintheta = fIFy*fIt**2*(2*fIeta/(1-fIbeta)+4/(1-fIbeta)**0.5)*fIQf
        return fOPnsintheta
