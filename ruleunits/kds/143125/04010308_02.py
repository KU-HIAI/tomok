...
class KDS143125_04010308_02 (RuleUnit):
    ...
    @rule_method
    def design_shear_strength(fOphiVn,fIphi,fIFy,fIAg) -> bool:
        """설계전단강도

        Args:
            fOphiVn (float): 설계전단강도
            fIphi (float): 저항계수
            fIFy (float): 핀의 항복강도
            fIAg (float): 부재의 총단면적

        Returns:
            bool: 강구조 연결 설계기준(하중저항계수설계법)  4.1.3.8 휨모멘트를 받는 핀의 설계전단강도 (2)의 값
        """
        fOphiVn = fIphi * 0.6 * fIFy * fIAg
        return fOphiVn
