...
class KDS143125_0403020103 (RuleUnit):
    ...
    ref_code = 'KDS 14 31 25 4.3.2.1.3' # 건설기준문서
    ref_date = '2017-12-20'  # 고시일
    doc_date = '2023-12-05'  # 건설기준문서->디지털 건설기준 변환 기준일
    title = 'K형 접합의 압축력을 받는 지강관의 설계강도'  # 건설기준명
  ...
  F-->N-->I
    ...
    @rule_method
    def Design_strength_of_steel_pipe_subjected_to_compressive_force_of_K_type_joint(fOphiPn,fIPnsintheta,fIPn) -> float:
        """K형 접합의 압축력을 받는 지강관의 설계강도

        Args:
            fOphiPn (float): T-및 Y-형 접합에서 주강관의 소성화의 한계상태
            fIPnsintheta (float): 주강관의 항복강도
            fIPn (float): 주강관의 두께

        Returns:
            float: 강구조 연결 설계기준(하중저항계수설계법)  4.3.2.1.3 K형 접합의 압축력을 받는 지강관의 값
        """
