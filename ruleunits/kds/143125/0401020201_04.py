...
class KDS143125_0401020201_04 (RuleUnit):
    ...
    ref_code = 'KDS 14 31 25 4.1.2.2.1 (4)' # 건설기준문서
    ref_date = '2017-12-20'  # 고시일
    doc_date = '2023-09-11'  # 건설기준문서->디지털 건설기준 변환 기준일
    title = '플러그용접과 슬롯용접의 유효길이'    # 건설기준명
    ...
    @rule_method
    def Effective_length_of_plug_welding_and_slot_weld(fOelpwsw,fIlenwcl) -> bool:
        """플러그용접과 슬롯용접의 유효길이

        Args:
            fOelpwsw (float): 플러그용접과 슬롯용접의 유효길이
            fIlenwcl (float): 용접중심선의 길이

        Returns:
            bool: 강구조 연결 설계기준(하중저항계수설계법)  4.1.2.2.1 필릿용접의 유효면적 (4)의 값
        """
