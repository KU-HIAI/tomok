...
class KDS143125_0403020202_03 (RuleUnit):
    ...
    ref_code = 'KDS 14 31 25 4.3.2.2.2 (3) ' # 건설기준문서
...
U --"Minimum"-->W(["<img src='https://latex.codecogs.com/svg.image?\phi&space;P_{n}'>--------------------"])
    ...
    @rule_method
    def Limit_state_of_local_surrender(fOPnsintheta,fIFy,fIt,fIQf,fIk,fIN,fIH,fIE,fIHb,fItheta,fIuserdefined) -> bool:
        """국부항복의 한계상태
        Args:
        ...
        Returns:
            bool: 강구조 연결 설계기준(하중저항계수설계법) 4.3.2.2.2 T, Y, X형 접합의 압축력을 받는 지강관 (3)의 값
        """

        # 국부항복의 한계상태 : fIuserdefined == 1
        # T, Y형 접합에서 측벽 국부크리플링의 한계상태 : fIuserdefined == 2
        # X형 접합에서 측벽 국부크리플링의 한계상태 : fIuserdefined == 3

        if fIuserdefined == 1:
          fIN = fIHb / (math.sin(math.radians(fItheta)))
          fOPnsintheta = 2*fIFy*fIt*(5*fIk+fIN)
        elif fIuserdefined == 2:
          fOPnsintheta = 1.6*fIt**2*(1+3*fIN/(fIH-3*fIt))*(fIE*fIFy)**0.5*fIQf
        elif fIuserdefined == 3:
          fOPnsintheta = (48*fIt**3/(fIH-3*fIt))*(fIE*fIFy)**0.5*fIQf
