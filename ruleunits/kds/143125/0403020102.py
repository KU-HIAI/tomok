...
class KDS143125_0403020102 (RuleUnit):
    ...
    title = 'T, Y, X형 접합의 압축력을 받는 지강관의 설계강도'  # 건설기준명
...
G-->E-->L-->M
    ...
    @rule_method
    def Design_strength_of_steel_pipes_subjected_to_compressive_force_of_T_Y_and_X_type_joints(fOphiPn,fIPnsintheta,fIPn,fIbeta,fIgamma) -> float:
        """T, Y, X형 접합의 압축력을 받는 지강관의 설계강도

        Args:
            fOphiPn (float): 지강관의 설계강도
            fIPnsintheta (float): 주강관의 소성화의 한계상태
            fIPn (float): 전단항복(뚫림)의 한계상태
            fIbeta (float): 폭비
            fIgamma (float): 주강관 세장비

        Returns:
            float: 강구조 연결 설계기준(하중저항계수설계법)  4.3.2.1.2 T, Y, X형 접합의 압축력을 받는 지강관의 값
        """

        if fIbeta > (1-1/fIgamma) :
          fOphiPn = fIPnsintheta*0.9
        else:
          fOphiPn = min(fIPnsintheta*0.9, fIPn*0.95)
