...
class KDS143125_0403020202_04 (RuleUnit):
    ...
    ref_code = 'KDS 14 31 25 4.3.2.2.2 (4) ' # 건설기준문서
...
U --"Minimum"-->W(["<img src='https://latex.codecogs.com/svg.image?\phi&space;P_{n}'>--------------------"])
    ...
    @rule_method
    def Limit_state_of_local_surrender(fOPn,fIFyb,fItb,fIHb,fIk,fIN,fIH,fIE,fIbeoi,fIB,fIt,fIFy,fIBb) -> bool:
        """국부항복의 한계상태
        Args:
            fOPn (float): 비균일 하중분포로 인한 국부항복의 한계상태
        ...
        Returns:
            bool: 강구조 연결 설계기준(하중저항계수설계법) 4.3.2.2.2 T, Y, X형 접합의 압축력을 받는 지강관 (4)의 값
        """

        fIbeoi = min((10/(fIB/fIt))*(fIFy*fIt)/(fIFyb*fItb)*fIBb, fIBb)
        fOPn = fIFyb*fItb*(2*fIHb+2*fIbeoi-4*fItb)
        return fOPn
