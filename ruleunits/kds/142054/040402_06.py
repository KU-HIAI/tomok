...
class KDS142054_040402_06 (RuleUnit):
    ...
    ref_code = 'KDS 14 20 54 4.4.2 (6)' # 건설기준문서
    ref_date = '2021-02-18'  # 디지털 건설문서 작성일
    doc_date = '2023-10-16'  # 건설기준문서->디지털 건설기준 변환 기준일
    title = '가장자리 효과에 대한 수정계수'    # 건설기준명
    ...
    A[전단을 받는 단일 앵커 또는 앵커 그룹의 가장자리 효과에 대한 수정계수];
    ...
    @rule_method
    def correction_factor_for_edge_effect(fOpsiedV,fIcaone,fIcatwo) -> float:
        """가장자리 효과에 대한 수정계수

        Args:
            fOpsiedV (float): 가장자리 효과에 대한 수정계수
            fIcaone (float): 앵커 샤프트 중심부터 콘크리트 단부까지의 거리
            fIcatwo (float): 앵커 샤프트 중심부터 ca1과 직각방향에 있는 콘크리트 단부까지 거리

        Returns:
            float: 콘크리트용 앵커 설계기준  4.4.2 전단력을 받는 앵커의 콘크리트 브레이크아웃강도 (6)의 값
        """
