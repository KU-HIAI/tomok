...
class KDS241711_04060604_03(RuleUnit): # KDS241711_04060604_03

    # 아래 클래스 멤버 변수에 할당되는 값들을 작성하는 룰에 맞게 수정
    priority = 1   # 건설기준 우선순위
    author = 'Chanwoo Yang'  # 작성자명
    ref_code = 'KDS 24 17 11 4.6.6.4 (3)' # 건설기준문서
    ref_date = '2022-02-25'  # 디지털 건설문서 작성일  (고시일)
    doc_date = '2023-10-17'  # 건설기준문서->디지털 건설기준 변환 기준일 (작성 년월)
    title = '단부구역의 공칭전단강도'   # 건설기준명
  ...
  flowchart TD
    ...
    Python_Class ~~~ Variable_def --단부구역-->D -->F
...
Variable_def --단부구역 제외--> E
   E(["KDS 24 17 11 4.6.2.6"]) ;
    ...
    @rule_method
    def nominal_shear_strength_of_end_zone(fOVn,fIVc,fIVs,fIVp) -> float:
        """단부구역의 공칭전단강도

        Args:
            fOVn (float): 단부구역의 공칭전단강도
            fIVc (float): 콘크리트에 의한 공칭전단강도
            fIVs (float): 전단철근에 의한 공칭전단강도
            fIVp (float): 축력 작용에 의한 공칭전단강도

        Returns:
            float: fOVn, 단부구역의 공칭전단강도
        """
