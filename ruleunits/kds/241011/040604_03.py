...
class KDS241011_040604_03(RuleUnit):
    ...
    ref_code = 'KDS 24 10 11 4.6.4 (3)' # 건설기준문서
    ref_date = '2021-04-15'  # 디지털 건설문서 작성일  (고시일)
    doc_date = '2023-10-16'  # 건설기준문서 -> 디지털 건설기준 변환 기준일 (작성 년월)
    title = '두 차로 이상 재하 된 경우 차로 당 전단력과 모멘트에 대한 교축방향 스트립의 등가폭'   # 건설기준명
   ...
   # 이미지 링크 변환 사이트 https://www.somanet.xyz/2017/06/blog-post_21.html
    ...
    @rule_method
    def fOE_040604_03(fOE,fILactsp,fIW,fINL) -> float:
        """두 차로 이상 재하 된 경우 차로 당 전단력과 모멘트에 대한 교축방향 스트립의 등가폭
        Args:
            fOE (float) : 등가폭
            fILactsp (float): 실제 지간장.
            fIW (float): 실제 교폭.
            fINL (float): KDS 24 12 21 4.3.1.1에 규정도니 재하차로 수.
        Returns:
            float: fOE, 등가폭
        """
        L1=min(fILactsp,18000)
        W1=min(fIW,18000)
        fOE=1.1*(2100+0.12*(L1*W1)**0.5)
        if fOE<=fIW/fINL:
          return(fOE)
        else:
          return("Fail")
