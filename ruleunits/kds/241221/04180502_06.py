...
class KDS241221_04180502_06 (RuleUnit):
    ...
    ref_code = 'KDS 24 12 21 4.18.5.2 (6)' # 건설기준문서
    ref_date = '2018-08-30'  # 고시일
    doc_date = '2023-11-27'  # 건설기준문서->디지털 건설기준 변환 기준일
    title = '통과경로에 직각방향 유속에 대한 보정계수'    # 건설기준명
    ...
    flowchart TD
        ...
        A[수로에서 선박의 통과경로에 직각방향 유속에 대한 보정계수];
    ...
    @rule_method
    def correction_factor_for_the_flow_rate_perpendicular_to_the_passage_path(fORxc,fIVxc) -> float:
        """통과경로에 직각방향 유속에 대한 보정계수

        Args:
            fORxc (float): 통과경로에 평행한 유속에 대한 보정계수
            fIVxc (float): 선박의 통과경로에 평행한 유속성분
        Returns:
            float: 강교 설계기준(한계상태설계법) 4.18.5.2 항로이탈확률 (6) 의 값
        """
