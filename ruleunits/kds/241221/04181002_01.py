...
class KDS241221_04181002_01 (RuleUnit):
    ...
    flowchart TD
        ...
        A[갑판실과의 충돌에 의하여 상부구조부에 미치는 충격력];
    ...
    @rule_method
    def impact_force_of_the_ship_deck_compartment(fOPdh,fIRdh,fIPs) -> float:
        """선박 갑판실의 충격력

        Args:
            fOPdh (float): 선박 갑판실의 충격력
            fIRdh (float): 감소계수
            fIPs (float): 식 (4.18-17)에서 규정된 선박의 충격력
        Returns:
            float: 강교 설계기준(한계상태설계법) 4.18.10.2 갑판실과의 충돌 (1) 의 값
        """
