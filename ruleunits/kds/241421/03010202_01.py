...
class KDS241421_03010202_01 (RuleUnit):
    ...
    ref_code = 'KDS 24 14 21 3.1.2.2 (1)' # 건설기준문서
    ref_date = '2021-04-12'  # 디지털 건설문서 작성일
    doc_date = '2023-11-24'  # 건설기준문서->디지털 건설기준 변환 기준일
    title = '보통콘크리트의 탄성계수'    # 건설기준명
    ...
    @rule_method
    def Modulus_of_elasticity_of_normal_concrete(fOEc,fIfcm,fImc) -> float:
        """보통 콘크리트의 탄성계수

        Args:
             fOEc (float): 보통 콘크리트의 탄성계수
             fIfcm (float): 콘크리트의 평균압축강도
             fImc (float): 콘크리트의 단위 질량


        Returns:
            float: 콘크리트교 설계기준 (한계상태설계법) 3.1.2.2 탄성변형 (1)의 값
        """
