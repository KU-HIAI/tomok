...
class KDS249011_04020309 (RuleUnit):
    ...
    ref_code = 'KDS 24 90 11 4.2.3.9' # 건설기준문서
    ...
    F["<img src='https://latex.codecogs.com/svg.image?\epsilon&space;_{q,d}=\frac{\nu&space;_{xy,d}}{T_{q}}'>--------------------------------------------------------"];
    ...
    @rule_method
    def Shear_Strain_Of_An_Elastomer_Due_To_Displacement(fOepsilonqd, fIvxyd, fITq) -> float:
        """전단변형률

        Args:
            fOepsilonqd (float): 이동 변위로 인한 탄성중합체의 전단변형률
            fIvxyd (float): vx,d와 vy,d의 벡터합으로 계산되는 받침의 수평상대변위
            fITq (float): 상하 덮개를 포함한 탄성중합체의 총 두께

        Returns:
            float: 교량 기타시설설계기준 (한계상태설계법)  4.2.3.9 전단변형률의 값
        """
