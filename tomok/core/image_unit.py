...
class ImageUnit(BaseUnit):
    ...
    def _regist_image_functions(self):
        method_list = [
            ...
            if typename(getattr(self, attr)) == "tomok.core.decorator.ImageFunction"
    ...
    def verify(self, with_stdin=True):
        ...
        if self.acc_able is not None:
            print(
                "\033[92m" + "[통과]" + "\033[0m" + " acc 가능여부가 설정되어 있습니다."
            )
        else:
            print(
                "\033[91m"
        ...
        for rule_method in self.image_functions:
            ...
            # 룰 인자 검증
            print("\033[1m[이미지 함수 입력 인자 검증]\033[0m")
            ...
            # 룰 실행여부 검증
            print("\033[1m[이미지 함수 실행 여부 검증]\033[0m")
            ...
            if with_stdin:  # input()의 stdin 입력 없이 검토하는 기능 추가
                ...
                try:
                    ...
                    print("\033[92m" + "[통과]" + "\033[0m" + " 이미지 실행 확인")
                except Exception as ex:
                    print("\033[91m" + "[오류]" + "\033[0m" + " 이미지 실행 실패")
                    print(ex)
                    error_flag = 1

                # 룰 반환형 검사
                print("\033[1m[이미지 함수 반환 데이터 검증]\033[0m")
                print("결과:", result)
            ...
            for extreme_args in islice(
                ...
                try:
                    ...
                    if not isinstance(output, ResultBase):
                        ...
                        raise TypeError("ImageFunction 반환형 오류")
