...
class RuleIFCController:
    def __init__(
        self, path="rules", rule_units: RuleUnitController = None, mode="local"
    ):
                        ...
                        if len(matches) > 0:
                            if self._is_valid_filename(filename):
                                for match in matches:
                                    ...
                                    rule = getattr(
                                        import_module(import_name, package=None),
                                        cls_name,
    ...
    def _is_valid_filename(cls, filename: str) -> bool:
        if filename.count(".") > 1:
            ...
            raise FileError(
                "rule 파일명에 . 글자는 허용되지 않습니다. 파일명에서 . 을 제거해주시기 바랍니다."
            )
        return True
    ...
    def verify_all(
        self, reader: IFCReader, guid: str = None, return_results: bool = False
    ):
