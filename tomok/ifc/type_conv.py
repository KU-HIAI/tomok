...
def ifc_single_value_to_python(
    value_type: str, wrapped_value: object, prop: object
) -> object:
    try:
        ...
        if type(wrapped_value) is float:
            ...
            raise TypeError(
                "currently {0} ({1}, {2}) type is not handled in IFCReader.".format(
                    value_type, wrapped_value, type(wrapped_value)
                )
            )
    except Exception as ex:
        raise ValueError(
            "parsing value ({0}) with {1} type is not possible. prop:{2}".format(
...
def python_to_ifc_single_value(
    ifc: file, name: str, value: object, value_type: str = None, description: str = None
):
...
def _recognize_value_type(value: object):
    ...
    if type(value) is int:
        ...
        raise TypeError(
            "currently {0} ({1}) type is not writable in IFCReader".format(
                type(value), value
            )
        )
