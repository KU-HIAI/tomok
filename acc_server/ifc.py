...
# framework
from tomok import IFCReader, RuleIFCController, RuleUnitController
...
def get_rules(user) -> Dict:
    ric = RIC.get()
    return {
        'rules': json.loads(ric.rules_df.reset_index().to_json(force_ascii=False,orient='records',indent=4)),
        'status': 200,
        'detail': 'ok'
    }
