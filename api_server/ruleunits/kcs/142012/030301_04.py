...
class KCS142012_030301_04(RuleUnit):
    ...
    def dismantling_sheathing(bIFauStr,bIMulStr,bIFilSho,fIFck) -> RuleUnitResult:
        """
        Args:
            bIFauStr (bool): 단층구조
            bIMulStr (bool): 다층구조
            bIFilSho (bool): 필러 동바리 구조
            fIFck (float): 설계기준 압축강도

        Returns:
            sODisShe (str): 슬래브 및 보의 밑면, 아치 내면의 거푸집널 해체 시기
        """
        assert isinstance(bIFauStr, bool)
        assert isinstance(bIMulStr, bool)
        assert bIFauStr != bIMulStr
        assert isinstance(bIFilSho, bool)
