...
flowchart TD
    ...
    @rule_method
    def ready_mixed_concrete_nominal_strength(fIConQua, fITemAdj) -> RuleUnitResult:
        """레디믹스트 콘크리트 호칭강도

        Args:
            fIConQua (float): 콘크리트 품질기준 강도
            fITemAdj (float): 기온보정강도

        Returns:
            fONomStr (float):레디믹스크 콘크리트 호칭강도
        """
        assert isinstance(fIConQua, float)
        assert isinstance(fITemAdj, float)
        assert fITemAdj in [0, 3, 6]
        assert 0 <= fIConQua

        if fITemAdj == 0:
            fONomStr = fIConQua + fITemAdj
