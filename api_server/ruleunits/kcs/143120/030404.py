...
class KCS143120_030404(RuleUnit):

    priority = 1
    acc_able = False
    author = '국가건설기준센터'
    ref_code = 'KCS 14 31 20 3.4.4'
    ref_date = '2019-05-20'
    doc_date = '2023-11-13'
    title = '플랜지 내·외면의 가열'
    ...
    content = """
    #### 3.4.4 가용접의 최소 예열온도
    가용접의 최소 예열온도 및 용접층간 온도는 아래의 표 3.4-1에 준한다.
    표 3.4-1 최소 예열온도(℃)
    \begin{table}[]
...
\end{table}
    ...
    B["KCS 14 31 20 3.4.4"];
    B ~~~ A
    end

    KCS(["KCS 14 31 20 3.4.4"])

    subgraph Variable_def
    VarOut1[/"출력변수: 가용접의 최소 예열온도 및 용접층간 온도"/];
    VarIn1[/입력변수: 강종/];
    VarIn2[/입력변수: 용접방법/];
    VarIn3[/입력변수: 판두께/];
		VarOut1 ~~~ VarIn1
    ...
    KCS --> Variable_def
    Variable_def --> C{"강종 \n 용접방법 \n 판두께"}
    C --> |"표 3.4-1"|D["가용접의 최소 예열온도 및 용접층간 온도"]
		D --> E(["가용접의 최소 예열온도 및 용접층간 온도"])
    """

    @rule_method
    def Steel_Grade(sIWelMet, sISteGra, fIThiPla) -> RuleUnitResult:
        """ 플랜지 내·외면의 가열
        Args:
            sIWelMet (str): 용접방법
            sISteGra (str): 강종
        ...
        Returns:
            sOPreInt (str): 가용접의 최소 예열온도 및 용접층간 온도
        """
