...
class KCS243000_030201_06(RuleUnit):
    ...
    def shape_specimen(fIW,fIT,fIB,fIL,fIR,fIStrCon,fIPerBen,fIStrLoa) -> RuleUnitResult:
        """
        Args:
            fIW (float): 단면폭
            fIT (float): 판두께
        ...
        Returns:
            pass_fail_1 (bool): 강교량공사 3.2.1 일반사항 (6) ③의 판단 결과
            pass_fail_2 (bool): 강교량공사 3.2.1 일반사항 (6) ④의 판단 결과
            pass_fail_3 (bool): 강교량공사 3.2.1 일반사항 (6) ⑤의 판단 결과
        """
        ...
        assert isinstance(fIPerBen, float)
        assert isinstance(fIStrLoa, float)
