...
class KCS243000_030104_03(RuleUnit):
    ...
    def hole_drilling(bISecMem,fIPlaThi, bISpeDra,sIBolNam,bISheSte,bIRolSte,bIHigBol,bIGenBol,bIDriFla,bIGenFla,bIBeaMaj) -> RuleUnitResult:
        """
        Args:
            bISecMem (bool): 2차부재
            fIPlaThi (float): 판두께
            bISpeDra (bool): 설계도에 특별히 지정
            sIBolNam (str): 볼트의 호칭
            bISheSte (bool): 전단연
            bIRolSte (bool): 자동가스 전단연, 압연연 및 다듬질한 연
            bIHigBol (bool): 고장력볼트
            bIGenBol (bool): 일반볼트
            bIDriFla (bool): 타입식고장력 접시머리형 볼트
            bIGenFla (bool): 보통접시 머리형 볼트
            bIBeaMaj (bool): 주요부재에 일반볼트를 지압접합으로 사용

        Returns:
            sODriSte (str): 강재에 구멍 뚫기
            sOBolEdg (str): 볼트 구멍중심에서 연단까지의 거리
            sODimHol (str): 볼트 구멍의 공칭치수
        """
        assert isinstance(bISecMem, bool)
        assert isinstance(fIPlaThi, float)

        assert isinstance(bISpeDra, bool)
        assert isinstance(sIBolNam, str)
        assert sIBolNam in ["M8", "M10", "M12", "M16", "M20", "M22", "M24", "M27", "M30"]
        assert isinstance(bISheSte, bool)
        assert isinstance(bIRolSte, bool)
        assert bIRolSte != bISheSte

        assert isinstance(bIHigBol, bool)
        assert isinstance(bIGenBol, bool)
        assert isinstance(bIDriFla, bool)
        assert isinstance(bIGenFla, bool)
        assert isinstance(bIBeaMaj, bool)
        assert (bIHigBol+bIGenBol+bIDriFla+bIGenFla+bIBeaMaj) <= 1
