...
class KCS243000_030104_09(RuleUnit):
    ...
    def steel_deck_construction(fIThiJoi,fID,fIT,fIL_1,fIL_2) -> RuleUnitResult:
        """
        Args:
        ...
        """
        assert isinstance(fIThiJoi, float)

        assert isinstance(fID, float)
        assert isinstance(fIT, float)

        assert isinstance(fIL_1, float)
        assert isinstance(fIL_2, float)
