...
class KCS244005_030203_02(RuleUnit):
    ...
    def elastometric_bearing(sITesIte,bIQuaSta,bIRem) -> RuleUnitResult:
        """
        Args:
            sITesIte (str): 검사항목
            bIQuaSta (bool): 품질기준
        ...
        Returns:
            sOBeaPer (str): 탄성받침 완제품 성능
        """
        assert isinstance(sITesIte, str)
        assert isinstance(bIQuaSta, bool)
        assert isinstance(bIRem, bool)
        assert bIQuaSta != bIRem
        assert sITesIte in ["전단계수", "저온 전단계수", "노화후 전단계수", "대기온도 전단부착", "노화후 전단부착",
                            "압축강도", "압축반복 재하(피로시험)", "편심재하시험", "복원모멘트시험", "오존저항시험"]
        ...
        if sITesIte == "전단계수":
            ...
            if bIQuaSta:
                sOBeaPer = "압축계수 증가율이 피로시험전의 12% 이내, ∙부착결함, 균열이 없어야 한다."
            elif bIRem:
                sOBeaPer = "시험반복횟수: 2,000,000회, 주파수 ＜ 3 ㎐, 응력변화는 7.5 MPa ~ 25 MPa 이내"
