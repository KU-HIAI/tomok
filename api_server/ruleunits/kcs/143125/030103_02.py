...
flowchart TD
    ...
    VarOut1[/"출력변수: 접합부 표면의 높이 차이 처리 방법(교량)"/]
    VarIn1[/입력변수: 높이 차이/];
    VarOut1 ~~~ VarIn1
    end
    ...
    @rule_method
    def method_for_joint_surface_difference_in_bridge(fIDifJoi) -> RuleUnitResult:
        ...
        Returns:
            sOJoiSur (str): 접합부 표면의 높이 차이 처리 방법(교량)
        """
        assert isinstance(fIDifJoi, float)
