...
class KDS241011_040604_02(RuleUnit):
    ...
    Python_Class ~~~ C(["KDS 24 10 11 4.6.4 (2)"])
		C --> Variable_def

    D["L1=Min(실제 지간장,18,000mm)"];
    G["W1=Min(실제 교폭, 9,000mm)"];
    ...
    def equivalent_width_of_longitudinal_strip(fILactsp,fIW,fINL) -> RuleUnitResult:
        ...
        Returns:
            fOE (float): 교량 설계 일반사항(한계상태설계법)  4.6.4 슬래브교에 대한 등가 스트립 폭 (2)의 값
            pass_fail (bool): 교량 설계 일반사항(한계상태설계법)  4.6.4 슬래브교에 대한 등가 스트립 폭 (2)의 판단 결과
        """
        ...
        if fINL == 1 :
          fOE = 1.2 * (250 + 0.42 * (min(fILactsp, 18000)*min(fIW, 9000))**0.5)
