...
class KDS143110_040203_02(RuleUnit):
    priority = 1
    acc_able = True
    author = '국가건설기준센터'
    ...
    @rule_method
    def buckling_stress(fOFcr,fOFe,fIFy,fIE,fIK,fIL,fIr) -> RuleUnitResult:
        """좌굴응력

        Args:
            fOFcr (float): 좌굴응력
            fOFe (float): 탄성좌굴해석을 통하여 구하는 탄성좌굴응력
            fIFy (float): 강재의 항복강도
            fIE (float): 강재의 탄성계수
            fIK (float): 유효좌굴길이계수
        ...
        Returns:
            fOFcr (float): 강구조부재설계기준(하중저항계수설계법)  4.2.3 비세장판 단면을 가진 부재의 휨좌굴에 대한 압축강도 (1)의 값
            pass_fail (bool): 강구조부재설계기준(하중저항계수설계법)  4.2.3 비세장판 단면을 가진 부재의 휨좌굴에 대한 압축강도 (1)의 판단 결과
        """
