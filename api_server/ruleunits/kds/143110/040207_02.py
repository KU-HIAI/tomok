...
class KDS143110_040207_02(RuleUnit):
    priority = 1
    acc_able = False
    author = '국가건설기준센터'
    ref_code = 'KDS 14 31 10 4.2.7 (2)'
    ref_date = '2017-12-20'
    doc_date = '2024-02-13'
    title = '좌굴응력'
    ...
    def Nominal_Compressive_Strength(fIE,fIQ,fIFe,fIFy,fIKLr) -> RuleUnitResult:
        ...
        Returns:
            fOFcr (float): 강구조부재설계기준(하중저항계수설계법)  4.2.7 세장판단면을 갖는 압축부재의 값 (2)의 값
            pass_fail (bool): 강구조부재설계기준(하중저항계수설계법)  4.2.7 세장판단면을 갖는 압축부재의 값 (2)의 통과여부 1
            sOnone (string): 강구조부재설계기준(하중저항계수설계법)  4.2.7 세장판단면을 갖는 압축부재의 값 (2)의 통과여부 2
        """
