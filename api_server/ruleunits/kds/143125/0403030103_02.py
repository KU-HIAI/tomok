...
class KDS143125_0403030103_02(RuleUnit):
    priority = 1
    acc_able = False
    author = '국가건설기준센터'
    ref_code = 'KDS 14 31 25 4.3.3.1.3 (2)'
    ref_date = '2017-12-20'
    doc_date = '2024-02-25'
    ...
    Python_Class ~~~ C(["KDS 14 31 25 4.3.3.1.3 (2)"])
		C --> Variable_def
    ...
    def Limit_state_of_shea_yielding_punching(fIFy,fIt,fIDb,fItheta) -> RuleUnitResult:
        ...
        Returns:
            fOMnsint (float): 강구조 연결 설계기준(하중저항계수설계법)  4.3.3.1.3 T, Y, X형 접합에서 지강관의 면외 휨모멘트 (2)의 값 1
            fOphi (float): 강구조 연결 설계기준(하중저항계수설계법)  4.3.3.1.3 T, Y, X형 접합에서 지강관의 면외 휨모멘트 (2)의 값 2
        """
        ...
        fOMnsint = 0.6*fIFy*fIt*fIDb**2*(3+math.sin(fItheta*math.pi/180)/(4*math.sin(fItheta*math.pi/180)**2))
        fOphi = 0.9
