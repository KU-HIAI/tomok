...
class KDS143125_0403020204_03(RuleUnit):
    ...
    acc_able = False
    author = '국가건설기준센터'
    ref_code = 'KDS 14 31 25 4.3.2.2.4 (3)'
    ref_date = '2017-12-20'
    doc_date = '2024-02-25'
    ...
    Python_Class ~~~ C(["KDS 14 31 25 4.3.2.2.4 (3)"])
		C --> Variable_def
    ...
    @rule_method
    def Design_strength_of_steel_pipe(fIFybi,fItbi,fIOv,fIHbi,fIBbi,fIbeov) -> RuleUnitResult:
        ...
        Returns:
            fOPhiPn (float): 강구조 연결 설계기준(하중저항계수설계법)  4.3.2.2.4 겹침 K형 접합에서 압축력을 받는 지강관 (3)의 값 1
            fOPn (float): 강구조 연결 설계기준(하중저항계수설계법)  4.3.2.2.4 겹침 K형 접합에서 압축력을 받는 지강관 (3)의 값 2
            pass_fail (bool): 강구조 연결 설계기준(하중저항계수설계법)  4.3.2.2.4 겹침 K형 접합에서 압축력을 받는 지강관 (3)의 판단 결과
        """
        ...
        assert isinstance(fIBbi, float)
        assert isinstance(fIbeov, float)

        if 80 <= fIOv < 100:
          fOPn = fIFybi * fItbi * (2 * fIHbi - 4 * fItbi + fIBbi + fIbeov)
          fOphiPn = fOPn * 0.95
